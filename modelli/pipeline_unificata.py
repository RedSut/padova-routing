...
def genera_dataset_unificato(
    ...
    if usa_traffico:
        ...
        G_periodi = {
            0: genera_traffico_realistico(G, centri_list, fattore_centro=2.0),  # Mattina
            1: genera_traffico_realistico(G, centri_list, fattore_centro=1.1),  # Pomeriggio
            2: genera_traffico_realistico(G, centri_list, fattore_centro=1.8),  # Sera
        }
    else:
...
def _prepara_struttura_vicini_universale(G_periodi: dict, df_train: pd.DataFrame, weight_attr: str, k_vicini: int = 4):
    ...
    peso_arco_arr = np.array(peso_arco_matrice).T  # Shape: (N, 4)
